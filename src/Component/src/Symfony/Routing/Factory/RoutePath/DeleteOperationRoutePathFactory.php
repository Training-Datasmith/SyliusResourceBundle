...
use Sylius\Resource\Metadata\Api\Api_Operation_Interface;
 ...
 * @experimental
 */
final readonly class Delete_Operation_Route_Path_Factory implements Operation_Route_Path_Factory_Interface
{
    ...
    public function create_route_path(Http_Operation $operation, string $root_path): string
    {
        ...
        $identifier = $operation->get_resource()?->get_identifier() ?? 'id';
        if ($operation instanceof Delete_Operation_Interface) {
            $path = $operation instanceof Api_Operation_Interface && 'delete' === $short_name ? '' : '/' . $this->path_segment_name_generator->get_segment_name($short_name ?? '', false);
            return sprintf('%s/{%s}%s', $root_path, $identifier, $path);
        }
